...
1. `beta_interval()` adds columns for the lower and upper bound of a confidence interval based on the beta distribution.
2. `logit()` transforms a value on a bound scale to an unbounded scale (e.x., percentages bound by `[0, 1]` -> `[-Inf, Inf]`).
3. `expit()` transforms a value on an unbounded scale to a bound scale (e.x., `[-Inf, Inf]` -> percentages bound by `[0, 1]`).
